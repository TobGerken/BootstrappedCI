...
addParamValue(p,'edges',1,@(x) validateattributes(x,{'numeric'},{})); % TG
...
% this is a matrix with all distance pairs
iid = distmat(x,p.maxdist);

% calculate squared difference between values of coordinate pairs
% this calculates the corresponding difference in the residual 
...
switch p.type
    case 'gamma'
        ...
        fvar     = @(x) 1./2 * mean(x);
        fvariance     = @(x) 1./2 * var(x);
        
        % distance bins
        if numel(p.edges) == 1 % TG
        ...
        if p.anisotropy
            ...
            S.variance = accumarray(ixedge,lam,[numel(edges) 1],fvariance,nan);     
        ...
        if numel(p.edges) == 1 % TG
            ...
            S.distance = ((edges(1:end-1)+edges(2:end))/2)';
        end
        S.val(end,:) = [];
        S.variance(end,:) = [];
        S.num(end,:) = [];
