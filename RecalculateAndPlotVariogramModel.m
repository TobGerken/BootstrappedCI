...
for cs = Cases % loop over cases
    ...
    %           N   =  Var.(model{1}).(level{1}).num(i_run,:);
                ...
                b_mod = nan(size(D.Gamma,1),3);
                tic 
                for i = 1:size(D.Gamma,1)
                    y = D.Gamma(i,:);
                    Weights = 1./(D.G_sigma(i,:)) ;
                    fun = @(b)(b(3)+b(2)*(1-exp(-distance./b(1)))-y).*sqrt(Weights);
                    b0 = [distance(end)*2/3/3 , max(y) 0];
                    lb = [0 0 0];
                    [b_mod(i,:)] = dofit(fun,b0, lb);
                end
                toc
                ...
                b_ = b_mod(1,:);
                Gamma_pred = (b_(3)+b_(2)*(1-exp(-distance./b_(1)))) ;%D.Gamma_pred.exponential(1,:);
                D.Gamma_pred_bound = Gamma_pred;
                Gamma_BS = (b_mod(2:end,3)+b_mod(2:end,2).*(1-exp(-distance./b_mod(2:end,1)))); %D.Gamma_pred.exponential(2:end,:);
                D.Gamma_pred_BS = Gamma_BS;
                ...
                save([DataDir 'VarOutput_' model{1} '_' Nme.(cs{1}){run} '_' level{1} '_new.mat'],'-struct','D')
                clear D
                x = [ distance 500];  
...
for cs = Cases % loop over cases
    ...
    for level = {'ABL','LFT','HFT'} % now loop categories and models
        for run = 1:length(Nme.(cs{1}))    
            ...
            for model = {'WRF', 'CT'}
                % now do actual plotting 
                try
                    D = load([DataDir 'VarOutput_' model{1} '_' Nme.(cs{1}){run} '_' level{1} '_new.mat']);
                    disp([DataDir 'VarOutput_' model{1} '_' Nme.(cs{1}){run} '_' level{1} '_new.mat'])
                catch
                    continue
                end
    ...
    %           N   =  Var.(model{1}).(level{1}).num(i_run,:);
                ...
                Nugget = D.Coefs_new(2:1001,3);
                Nugget_Data = D.Coefs_new(1,3);
...
for cs = Cases % loop over cases
    ...
    %           N   =  Var.(model{1}).(level{1}).num(i_run,:);
                ...
                Sill = D.Coefs_new(2:1001,2);
                %Sill_Data = D.Coefs_new(1,2);
                
                Range = D.Coefs_new(2:1001,1)*3 ;
                %Range_Data = D.Coefs.exponential(1,1)*3 ;
                
                %Sill = D.Coefs.exponential(2:1001,2) ;
                %Range = D.Coefs.exponential(2:1001,1)*3 ;

                
                Sill_Data.(model{1}) = D.Coefs_new(1,2);
                Range_Data.(model{1}) = D.Coefs_new(1,1)*3 ;
                %Nugget_Data.(model{1}) = D.Coefs_new(1,3) ;
                
                %Sills.(model{1}).(Nme.(cs{1}))(ct) = D.Coefs_new(2:1001,2) ;
                %Ranges.(model{1}).(Nme.(cs{1}))(ct) = D.Coefs_new(2:1001,1)*3 ;
                %Nuggets.(model{1}).(Nme.(cs{1}))(ct) = D.Coefs_new(2:1001,3) ;
...
while repeat
   try
      x = lsqnonlin(fun,x0.*modifier(pp,:),xlb);
      repeat = false;
   catch
      pp = pp+1;
...
%% Get statistics summer ABL


D = load([DataDir 'VarOutput_WRF_SUM_ABL_new.mat']);
Ranges =  D.Coefs_new(:,1)*3
Sills =   D.Coefs_new(:,2)
Nuggets = D.Coefs_new(:,3)

figure('units','inches','position',[0 0 8 4])
subplot(3,2,1)
hold on; box on;
title('Bootstrapped Values: WRF SUM ABL')
text(0.01,0.9,'Range (km)','units','normalized')
R_CDF = histogram(Ranges(2:end),[0:2:1000000],'normalization','cdf','edgecolor','none')
line(Ranges(1)*[1 1], [0 1],'color','r')
text(Ranges(1), 0.5, num2str(Ranges(1),5), 'color','r')
set(gca,'xscale','log','xlim',[10 10000],'ylim',[0.001 1], 'ytick',[0 0.25 0.5 0.75 0.9 1],'ygrid','on')

subplot(3,2,2)
R_CDF = histogram(Ranges(2:end),[0:2:1000000],'normalization','cdf','edgecolor','none')
set(gca,'xscale','log','xlim',[1000 1000000],'ylim',[0.95 1], 'ytick',[0.95:0.01:1],'ygrid','on')

subplot(3,2,3)
hold on; box on;
text(0.01,0.9,'Sill (ppm^2)','units','normalized')
S_CDF = histogram(Sills(2:end),[0:10000],'normalization','cdf','edgecolor','none')
line(Sills(1)*[1 1], [0 1],'color','r')
text(Sills(1), 0.7, '  Data:', 'color','r')
text(Sills(1), 0.5, num2str(Sills(1),4), 'color','r')

set(gca,'xscale','log','xlim',[10 1000],'ylim',[0.001 1], 'ytick',[0 0.25 0.5 0.75 0.9 1],'ygrid','on')
ylabel('CDF')
subplot(3,2,4)
S_CDF = histogram(Sills(2:end),[0:10000],'normalization','cdf','edgecolor','none')
set(gca,'xscale','log','xlim',[100 10000],'ylim',[0.95 1], 'ytick',[0.95:0.01:1],'ygrid','on')

subplot(3,2,5)
hold on; ; box on;
text(0.01,0.9,'Nugget (ppm^2)','units','normalized')
R_CDF = histogram(Nuggets(2:end),[0:0.25:1000],'normalization','cdf','edgecolor','none')
line(Nuggets(1)*[1 1], [0 1],'color','r')
text(Nuggets(1), 0.5, num2str(Nuggets(1),4), 'color','r')

set(gca,'xscale','log','xlim',[0.1 20],'ylim',[0.001 1], 'ytick',[0 0.25 0.5 0.75 0.9 1],'ygrid','on')
xlabel('Value')
subplot(3,2,6)
R_CDF = histogram(Nuggets(2:end),[0:0.25:10000],'normalization','cdf','edgecolor','none')
set(gca,'xscale','log','xlim',[10 1000],'ylim',[0.95 1], 'ytick',[0.95:0.01:1],'ygrid','on')
xlabel('Value')
